...
export default function TitleView() {
  // ▼ Contextから直接データを取得
  const { hasSaveData, handleStartConnection, handleResetData, currentSeason, clearedData } = useSaveData();
    ...
    <div className={`h-[100dvh] w-full ${theme.bg} flex flex-col items-center justify-center p-4 font-sans relative overflow-hidden transition-colors duration-1000`}>
      ...
      {isSeason3 && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-hidden z-0 opacity-10">
          <div className="text-[30vw] font-serif font-black text-rose-600 tracking-tighter whitespace-nowrap">M.C.</div>
        </div>
      )}

      {/* Season 3 クリア後の雨エフェクト */}
      {isPostReichenbach && (
      ...
      <div className="z-10 flex flex-col items-center text-center animate-in fade-in zoom-in-95 duration-1000 ease-out w-full max-w-md">
        <h2 className={`${theme.textAccent} font-mono text-xs sm:text-sm tracking-[0.3em] mb-4 drop-shadow-md ${isSeason3 ? 'animate-pulse' : ''}`}>{theme.systemStatus}</h2>
        <h1 className={`text-5xl sm:text-7xl font-serif font-bold tracking-widest mb-2 border-b pb-4 ${isPostReichenbach ? 'text-zinc-300 border-zinc-700' : isSeason3 ? 'text-rose-100 border-rose-900' : 'text-[#f4ebd8] border-[#5c4d43]'}`}>TETHER</h1>
        <p className={`font-serif italic text-sm sm:text-base mb-16 max-w-sm ${isPostReichenbach ? 'text-zinc-600' : isSeason3 ? 'text-rose-500 font-bold tracking-widest' : 'text-[#8c7a6b]'}`}>{theme.subtitle}</p>

        <button onClick={handleStartConnection} className={`group relative px-8 py-4 bg-transparent border ${theme.borderAccent} ${theme.buttonText} font-bold font-mono tracking-widest rounded-full ${theme.buttonHoverBg} ${theme.buttonHoverText} ${theme.buttonHoverBorder} transition-all duration-300 active:scale-95 ${theme.shadow} backdrop-blur-sm w-full sm:w-auto`}>
          {hasSaveData ? '> RESUME CONNECTION' : '> INITIALIZE TETHER'}
        </button>

        {hasSaveData && (
          <button onClick={handleResetData} className={`mt-12 text-[10px] sm:text-xs font-mono transition-colors underline decoration-dotted tracking-widest active:scale-95 ${isPostReichenbach ? 'text-zinc-700 hover:text-zinc-400' : 'text-[#5c4d43] hover:text-rose-600'}`}>
            [ DEBUG: FORMAT SYSTEM ]
          </button>
        )}
        ...
        {isPWA && !hasSaveData && (
          <div className="mt-8 p-3 bg-amber-900/20 border border-amber-700/50 rounded-lg text-left animate-in fade-in slide-in-from-bottom-2">
            ...
            <p className="text-xs text-amber-100/80 leading-relaxed font-serif">ブラウザからアプリへ移行した探偵諸君へ。以前のデータは「ブラウザ側のアーカイブ」からエクスポートし、INITIALIZE後に「DATA IMPORT」で復元したまえ。</p>
          </div>
        )}
      </div>
    </div>

...
export default function EndRollView({ unlockedTerms, clearedData, insightPoints, onBackToTitle }: EndRollViewProps) {
      ...
      {phase === 1 && (
        <div className={`max-w-xl w-full px-8 transition-opacity duration-1000 ${letterVisible ? 'opacity-100' : 'opacity-0'}`}>
          <div className="space-y-6 text-sm sm:text-base leading-loose text-[#d8c8b8] tracking-widest">
            <p className="animate-in fade-in slide-in-from-bottom-2 duration-1000 delay-[500ms] fill-mode-both">
              親愛なるワトスンへ。
            </p>
            <p className="animate-in fade-in slide-in-from-bottom-2 duration-1000 delay-[2500ms] fill-mode-both">
              サセックスの海風は冷たいが、私の蜜蜂たちは元気にしている。<br />
              ロンドンの喧騒も、かつての凶悪な犯罪者たちも、今では遠い昔の幻のようだ。
            </p>
            <p className="animate-in fade-in slide-in-from-bottom-2 duration-1000 delay-[5000ms] fill-mode-both">
              君が残してくれた数々の記録を読み返すたび、<br />
              私は自分の不完全な論理が、君という温かい人間性にどれほど救われていたかを痛感している。
            </p>
            <p className="animate-in fade-in slide-in-from-bottom-2 duration-1000 delay-[8000ms] fill-mode-both">
              時代は変わり、論理だけでは解決できない不条理な暴力が世界を覆っている。<br />
              だが、我々が守り抜いたものは、君の記録と共に、確かに次の世代へ受け継がれていくはずだ。
            </p>
            <p className="animate-in fade-in slide-in-from-bottom-2 duration-1000 delay-[11500ms] fill-mode-both">
              長い間、本当にありがとう。<br />
              君は最高の相棒だった。
            </p>
          ...
          <div className="mt-16 text-center animate-in fade-in duration-1000 delay-[17000ms] fill-mode-both">
            <button 
              onClick={showResult}
              className="text-[#8c7a6b] hover:text-[#f4ebd8] text-sm tracking-widest flex items-center gap-2 mx-auto transition-colors"
            >
              手紙を閉じる <ChevronRight size={16} />
            </button>
          </div>
        </div>
      )}
      ...
      {phase === 2 && (
        <div className="max-w-md w-full px-6 animate-in zoom-in-95 fade-in duration-1000">
          <div className="text-center mb-10">
            <h1 className="text-3xl font-bold tracking-[0.2em] mb-2 uppercase border-b border-[#8c7a6b]/30 pb-4 inline-block">
              FINAL RESULT
            </h1>
          ...
          <div className="mt-12 text-center">
            {unlockPercentage < 100 && (
              <p className="text-[#8c7a6b] text-xs mb-4 tracking-widest">
                ※まだ解読されていない大索引データが存在します。
              </p>
            )}
            <button 
              onClick={onBackToTitle}
              className="bg-[#d8c8b8] text-[#1a1512] hover:bg-white font-bold py-3 px-8 rounded-full tracking-widest transition-transform active:scale-95 shadow-lg w-full sm:w-auto"
            >
              TITLE MENU へ戻る
            </button>
          </div>

...
function GameContent({ scenarioData }: { scenarioData: any }) {
  ...
  return (
    <div className={`w-full max-w-2xl mx-auto relative flex flex-col h-[100dvh] touch-manipulation overscroll-none transition-transform duration-75 select-none ${screenEffect === 'shake' ? '-translate-x-2' : ''} ${isSanityZero ? 'bg-[#1a0f0f]' : 'bg-[#f4ebd8]'}`}>
      ...
      <div className={`flex-1 flex flex-col relative overflow-hidden cursor-pointer transition-colors duration-1000 ${isSanityZero ? 'bg-[#1a0f0f]' : 'bg-[#f4ebd8]'}`} onClick={() => { if (!isScrollingRef.current && (Date.now() - lastActionTimeRef.current > 300)) { lastActionTimeRef.current = Date.now(); isStreaming ? skipStream() : nextBeat(); } }} onTouchStart={(e) => { isScrollingRef.current = false; touchStartRef.current = { x: e.touches[0].clientX, y: e.touches[0].clientY }; }} onTouchMove={(e) => { if (touchStartRef.current && (Math.abs(e.touches[0].clientX - touchStartRef.current.x) > 10 || Math.abs(e.touches[0].clientY - touchStartRef.current.y) > 10)) isScrollingRef.current = true; }}>
      ...
      {!isInterruptMode && collectedEvidence.length > 0 && (
        <div className={`p-3 flex flex-wrap gap-2 shrink-0 shadow-[0_-4px_10px_rgba(0,0,0,0.1)] overflow-x-auto custom-scrollbar border-t z-20 relative ${isSanityZero ? 'bg-[#2a0f15] border-rose-900/50' : 'bg-[#2a2420] border-[#3a2f29]'}`}>
          ...
          {collectedEvidence.map((ev) => (
            <button key={ev} onClick={(e) => { e.stopPropagation(); handleSelectEvidence(ev); }} disabled={isStreaming} className={`whitespace-nowrap text-[10px] sm:text-xs px-2.5 py-1.5 rounded-full font-bold transition-all active:scale-95 z-20 relative ${selectedEvidence === ev ? isSanityZero ? 'bg-rose-600 text-[#1a0f0f] shadow-[0_0_10px_rgba(225,29,72,0.4)]' : 'bg-amber-500 text-[#3a2f29] shadow-[0_0_10px_rgba(245,158,11,0.4)]' : isSanityZero ? 'bg-rose-950 text-rose-200 hover:bg-rose-900' : 'bg-[#5c4d43] text-[#f4ebd8] hover:bg-[#8c7a6b]'} disabled:opacity-50 disabled:cursor-not-allowed`}>{ev}</button>
          ))}
        </div>
      )}
